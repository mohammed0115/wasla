...
class InitiatePaymentUseCase:
    ...
    def execute(cmd: InitiatePaymentCommand) -> PaymentRedirect:
        ...
        redirect = gateway.create_intent(
            order=order,
            amount=order.total_amount,
            currency=order.currency or cmd.tenant_ctx.currency,
        ...
        if gateway.code == "dummy":
            ...
            Payment.objects.create(
                ...
                status="success",
                amount=order.total_amount,
                reference=intent.provider_reference or idempotency_key,
            )
            CreditOrderPaymentUseCase.execute(CreditOrderPaymentCommand(order_id=order.id))
            NotifyMerchantOrderPlacedUseCase.execute(
                NotifyMerchantOrderPlacedCommand(order_id=order.id, tenant_id=cmd.tenant_ctx.tenant_id)
            )
            TelemetryService.track(
            ...
            return PaymentRedirect(redirect_url=cmd.return_url, client_secret=None, provider_reference=intent.provider_reference)
