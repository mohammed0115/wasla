...
class PaymentsReadinessChecker:
    ...
    def run(self) -> list[GoLiveCheckItem]:
        ...
        active_gateway_ok = gateway_settings.exclude(provider_name="").exists()
        items.append(
            GoLiveCheckItem(
                ...
                else "Configure at least one enabled gateway (mode=gateway) with provider name.",
                category=self.category_key,
            )
        )

        webhook_ok = gateway_settings.exclude(webhook_secret="").exists()
        items.append(
            GoLiveCheckItem(
